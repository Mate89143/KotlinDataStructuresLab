...
fun reto23() {
    val calorias = listOf(2000, 2100, 1900, 2200, 2050, 1950, 2300)
    ...
    for (i in calorias.indices) {
        if (i == 0) {
            ...
            val promedio = suma.toDouble() / i
            diferencias.add(calorias[i] - promedio)
        }
    }
    println("=== RETO 23 ===")
    println("Calorías diarias: $calorias")
...
fun reto30() {
    ...
    val columnas = layout[0].size
    val transpuesta = Array(columnas) { Array(filas) { "" } }
    ...
    println("=== RETO 30 ===")
    println("Layout original 3x2:")
    for (fila in layout) println(fila.joinToString())
    println("Layout transpuesto 2x3:")
